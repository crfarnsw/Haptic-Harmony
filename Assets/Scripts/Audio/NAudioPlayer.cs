...
{
    public static AudioClip FromMp3Data(FileInfo info)
    {
        // Load the data into a stream
        using (MemoryStream mp3Stream = new MemoryStream(File.ReadAllBytes(info.FullName)))
        {
            // Convert the data in the stream to WAV format
            using (Mp3FileReader mp3Audio = new Mp3FileReader(mp3Stream))
            {
                //TODO: Try DMO audio decoder
                ...
                AudioClip audioClip = AudioClip.Create(info.Name, wav.SampleCount, 2, wav.Frequency, false);
